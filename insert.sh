...
function PK() {
        ...
        if [[ $ispk == [Yy] ]]
        then
                check=`awk -F: 'BEGIN{flag=0}{ if($i==$field_value){flag=1; break}} END{print $flag}' $tabel_name`
		return $check
fi
}
function append() {
...
function checktype() {
	...
	#start check
                if [[ $ispk == [Yy] ]]
                then
                        check=`awk -F: 'BEGIN{flag=0}{ if($i==$field_value){flag=1; exit}} END{print $flag}' $tabel_name`
                fi
		# end check
		#start add
		if [[ $check == 3 || $check == 0 ]]
